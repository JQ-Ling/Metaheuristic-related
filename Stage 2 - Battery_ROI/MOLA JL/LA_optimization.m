function [XPF, FVALPF, PF_log] = LA_optimization(fhandle,d,pop,LB,UB,ref,n_iter,Np,Rc,S_c,M,ngrid,Nr,IntCon,fnonlin)
    for i=1:pop
    ...
    for t=1:n_iter
        ...
        PF_log{t} = REP.pos_fit;   % raw repository objectives this iter
                        ...
                        function REP = deleteFromRepository(REP,n_extra,ngrid)
                            ...
                            function [X]=LA_points(K,LB,UB,x0,scale_factor,d)
                                ...
                                if d > 3
                                    for i=1:2:d+2
                                        gama = rand; 
                                        R = [cos(gama) -sin(gama); sin(gama) cos(gama)];
                                        Xi(:,i:i+1) = (R*K')';
                                    end
                                    Xi = Xi(:,1:d);
                                end
