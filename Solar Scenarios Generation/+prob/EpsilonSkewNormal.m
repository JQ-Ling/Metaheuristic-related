classdef EpsilonSkewNormal < prob.ToolboxFittableParametricDistribution
...
%
%    An object of the EpsilonSkewNormal class represents a Laplace
...
%    DistributionName is the name of this distribution.
        DistributionName = 'epsilon-skew-normal';
    end
...
%    values of this distribution.
    ...
    methods
        ...
        function pd = EpsilonSkewNormal(mu,sigma)
            if nargin==0
                mu = 0;
                sigma = 1;
            end
...
%FIT Fit from data
%   P = prob.EpsilonSkewNormal.fit(x)
%   P = prob.EpsilonSkewNormal.fit(x, NAME1,VAL1, NAME2,VAL2, ...)
...
%                         the STATSET function
            ...
            x = prob.ToolboxFittableParametricDistribution.removeCensoring(x,cens,freq,'epsilon-skew-normal');
            freq = ones(size(x));
            ...
            pd = prob.EpsilonSkewNormal(m,s);
            
            % Fill in remaining properties defined above
            pd.ParameterIsFixed = [false false];
            [nll,acov] = prob.EpsilonSkewNormal.likefunc([m s],x);
        ...
        % parameter values stored in a EpsilonSkewNormal object. For
        ...
        function [nll,acov] = likefunc(params,x) % likelihood function
            ...
            nll = -sum(log(prob.EpsilonSkewNormal.pdffunc(x,mu,sigma)));
        ...
        function y = randfunc(mu,sigma,varargin) % random number generator
            y = prob.EpsilonSkewNormal.invfunc(rand(varargin{:}),mu,sigma);
        end
    end
    methods(Static,Hidden)
        ...
        function info = getInfo
            ...
            info = getInfo@prob.ToolboxFittableParametricDistribution('prob.EpsilonSkewNormal');
            
            % Then override fields as necessary
            info.name = 'epsilon-skew-normal';
            info.code = 'epsilon-skew-normal';
