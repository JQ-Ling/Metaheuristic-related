function [top5_loc,finalRank, composite_score] = R_method(Pareto_font,rank_obj)
    ...
    % Normalize across each column (Sum_N)
    Sum_N = sum(weightsVar, 1); % sum for each column
    W = weightsVar ./ Sum_N;    % normalize each column

    composite_score = W * weightsObj;
    % disp("YES")
    % composite_score = weightsVar * weightsObj;
    finalRank = tiedrank(composite_score);
    [~, sortedIndices] = sort(finalRank, 'descend');
    top5_loc = sortedIndices(1:5);
    % composite_score = composite_score(top5_loc);
end