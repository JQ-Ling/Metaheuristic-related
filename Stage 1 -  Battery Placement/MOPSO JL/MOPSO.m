...
function REP = MOPSO(params,MultiObj)
    ...
    gen      = 1;
    PF_log = cell(maxgen, 1);
    
    % Plotting and verbose
    % if(size(POS_fit,2)==2)
    %     h_fig = figure(1);
    %     h_par = plot(POS_fit(:,1),POS_fit(:,2),'or'); hold on;
    %     h_rep = plot(REP.pos_fit(:,1),REP.pos_fit(:,2),'oy'); hold on;
    %     try
    ...
    %     h_rep = plot3(REP.pos_fit(:,1),REP.pos_fit(:,2),REP.pos_fit(:,3),'oy'); hold on;
    %     try
    ...
    while ~stopCondition
        ...
        PF_log{gen} = REP.pos_fit;   % raw repository objectives this gen
