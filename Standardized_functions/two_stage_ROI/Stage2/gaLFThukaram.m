...
max_it = 200;
 while eps>ep
    % assert(it < max_it, 'Load flow did not converge within the maximum number of iterations.');
       it=it+1;
       Vpr=V;
       for ii=1:n
 ...
 %%% load flow output data
        ...
        result.infeasible = 0; % Default to feasible
        % if it == max_it
        %     result.Lineloss=Lineloss*1e6; % Assign heavy penalty for non-convergence
        %     % You may want to set result.infeasible = 1 here to tell MOMSA to reject this particle
        % end
%%% load flow output data
%return